...
-X
   ...
   * "-X dev": 启用 CPython 的“开发模式”，引入额外的运行时检测，这些检
     测因开销过大而无法默认启用。 如果代码是正确的则它不会比默认输出更
     详细：新增警告只会在发现问题时才会发出。 开发模式的作用效果：

     * 添加 "default" 警告过滤器，即 "-W" "default"。

     * 在内存分配器上安装调试钩子：参见 "PyMem_SetupDebugHooks()" C 函
       数。
...
PYTHONDEVMODE
   ...
   如果此环境变量被设为一个非空字符串，则会启用 CPython “开发模式”。
   参见 "-X" "dev" 选项。
