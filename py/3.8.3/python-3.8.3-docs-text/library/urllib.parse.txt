...
urllib.parse.urlparse(urlstring, scheme='', allow_fragments=True)
   ...
   *path* component, which is retained if present.  For example:
   ...
   invalid port is specified in the URL.  See section 结构化解析结果
   ...
   在 3.8 版更改: Characters that affect netloc parsing under NFKC
...
urllib.parse.quote(string, safe='/', encoding=None, errors=None)
   ...
   *string* may be either a "str" or a "bytes".
   ...
   在 3.7 版更改: Moved from **RFC 2396** to **RFC 3986** for quoting
   URL strings. "~" is now included in the set of unreserved
   characters.
